...
title Windows Troubleshooting & Diagnostics Tool
...
if "%netchoice%"=="1" (ipconfig /flushdns & pause & goto NETWORK_MENU)
if "%netchoice%"=="2" (netsh int ip reset & netsh winsock reset & echo Please restart after script finishes. & pause & goto NETWORK_MENU)
if "%netchoice%"=="3" (ipconfig /release & ipconfig /renew & pause & goto NETWORK_MENU)
if "%netchoice%"=="4" goto MAIN_MENU
goto NETWORK_MENU
...
if "%syschoice%"=="1" (sfc /scannow & pause & goto SYSTEM_MENU)
if "%syschoice%"=="2" (dism /online /cleanup-image /restorehealth & pause & goto SYSTEM_MENU)
...
if "%diskchoice%"=="1" (chkdsk C: /f & echo Disk will be checked on next reboot. & pause & goto DISK_MENU)
if "%diskchoice%"=="2" (
    del /q /s /f %temp%\*
